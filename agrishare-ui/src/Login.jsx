...
const Login = ({ onLogin, onNavigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        setErrors({ ...errors, email: data.error || 'Erreur de connexion' });
      } else {
      ...
      <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-8" style={{ width: '100%', maxWidth: '448px', minWidth: '320px' }}>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm font-medium">
            La plateforme intelligente des agriculteurs
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-between text-sm">
            <label className="flex items-center gap-2 cursor-pointer group">
              <div className="relative flex items-center">
                <input
                  ...
                  disabled={isLoading}
                />
                <div className="w-5 h-5 border-2 border-gray-300 rounded-md peer-checked:bg-[#2D6A4F] peer-checked:border-[#2D6A4F] transition-colors"></div>
                <svg
                  className="absolute w-5 h-5 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity"
                  viewBox="0 0 20 20"
                  fill="currentColor"
                >
            ...
            <a href="#" className="font-semibold text-[#2D6A4F] hover:text-[#1b4332] transition-colors">
              Mot de passe oublié ?
            </a>
          </div>
